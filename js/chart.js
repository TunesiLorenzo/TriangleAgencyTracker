export function createCharts(options = {}) {	
  ...
  function resetEffectTimer(){
    ...
    effectTimer = setTimeout(()=>{ meritMode=false; demeritMode=false; }, 10000);
  }
  ...
  function updateUnifiedData(){
    ...
    histData.forEach(row => row.histScale = demeritMode ? 1.2 : 1.0);
  ...
  return {
    start, stop,
    triggermeritMode, triggerdemeritMode, resetEffectTimer,
    // allow external mutation/read of internals if needed
    getState(){ return { meritMode, demeritMode }; },
